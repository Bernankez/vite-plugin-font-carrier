...
import BTT from "./assets/biantaoti.woff";
...
async function loadFont1() {
  const font = new FontFace("custom-font1", `url("${BTT}")`);
  await font.load();
  document.fonts.add(font);
}
